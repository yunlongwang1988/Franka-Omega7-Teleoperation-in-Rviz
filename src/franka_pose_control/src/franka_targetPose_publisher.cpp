...
private:
    void publishTargetPose()
    {
        ...
        target_pose_msg.position.x = 0.3 + 2 * pose_received_.pose.position.x; // 0.28
        target_pose_msg.position.y = 0 + 2 * pose_received_.pose.position.y;   // 0
        target_pose_msg.position.z = 0.6 + 2 * pose_received_.pose.position.z; // 0.6
        ...
        RCLCPP_INFO(this->get_logger(), "Target Pose Published!!");
