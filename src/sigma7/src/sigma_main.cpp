...
{
    return (a_val < a_minVal) ? a_minVal : (a_maxVal < a_val) ? a_maxVal
                                                              : a_val;
}
...
private:
    ...
    sensor_msgs::msg::Joy buttons_msg; // two elements, 0 is gripper button, 1 is pedal
...
int holdOnCenter(SigmaDevice *sigma_device)
{
    ...
    while (true)
    {
        ...
        double time = dhdGetTime();             // 获取程序启动后的当前时间
        if (time - lastDisplayUpdateTime > 0.1) // 当相邻时间大于0.1s时，刷新终端中的按钮状态，当循环频率比此高时，信息被淹没
        {
            lastDisplayUpdateTime = time; // 更新当前时间
        ...
        if (dhdKbHit() && dhdKbGet() == 'q')
        {
            std::cout << std::endl
                      << std::endl
                      << "exiting at user's request" << std::endl;
            break;
        }

        // 循环末尾添加输出信息控制频率
        rclcpp::sleep_for(std::chrono::milliseconds(1)); // 1000Hz
    }
...
int main(int argc, char *argv[])
{
    ...
    double rate;
    node->declare_parameter<double>("frequency", 1000);
